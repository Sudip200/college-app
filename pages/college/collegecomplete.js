...
const CompleteRegisterScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (!id) navigation.replace('College Login');
  }, [id]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/college/details/${id}`, {
        method: 'POST',
        body: formData,
      });
      const result = await res.json();
      navigation.replace('College Login');
    } catch (err) {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        <Text style={styles.title}>Complete College Profile</Text>
        ...
        <TouchableOpacity style={styles.logoButton} onPress={pickImage}>
          <Text style={styles.logoButtonText}>Upload College Logo</Text>
        </TouchableOpacity>
