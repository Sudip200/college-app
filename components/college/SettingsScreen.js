...
const SettingsScreen = ({ navigation, route }) => {
  ...
  const fetchAccountDetails = async () => {
     try{
      let res = await fetch(`${API}/college/${id}`,{
        method:'GET',
  ...
  const handleShare = async () => {
    try {
      await Share.share({
        message: `Join ${accountDetails.name} - Register here: ${API}/college/check/${id}`,
        title: `${accountDetails.name} Student Registration`,
        url: `${API}/college/check/${id}`
      });
    } catch (error) {
      ...
      <ScrollView style={styles.scrollContainer}>
        ...
        <View style={styles.settingsSection}>
          ...
          <SettingItem
            ...
            subtitle="Share link with students to register"
            onPress={handleShare}
